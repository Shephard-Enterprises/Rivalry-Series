...
function App() {
  ...
  const sounds = useGameSounds(social.profile, social.week)
  ...
  return <AuthGate><main className="app-shell">
    ...
    <div className="app-actions"><button className={sounds.enabled ? 'sound-button active' : 'sound-button'} type="button" onClick={() => { if (!sounds.enabled) sounds.preview(); sounds.toggle() }} aria-label={sounds.enabled ? 'Mute game sounds' : 'Turn on game sounds'} title={sounds.enabled ? 'Sounds on' : 'Sounds off'}>{sounds.enabled ? '🔊' : '🔇'}</button><button className="chat-button" type="button" onClick={() => setView('chat')} aria-label="Open rivalry chat">💬</button><button className="notification-button" type="button" onClick={() => setNotificationsOpen(true)} aria-label={`${social.unreadCount} unread notifications`}>♢{social.unreadCount > 0 && <span>{social.unreadCount > 9 ? '9+' : social.unreadCount}</span>}</button><button className="sign-out-button" type="button" onClick={() => supabase?.auth.signOut()}>Sign out</button></div>
