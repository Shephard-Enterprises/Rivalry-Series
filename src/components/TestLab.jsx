...
export default function TestLab({ onBack }) {
  ...
  const load = useCallback(async () => {
    ...
    if (!week) { setState({ week: null, profiles: profiles ?? [], players: [], picks: [], captains: [], scores: [], results: [], matchup: [], probabilities: [], events: [], recap: null }); return }
    const [playerResult, pickResult, captainResult, scoreResult, resultResult, matchupResult, probabilityResult, eventResult, recapResult] = await Promise.all([
      ...
      supabase.from('matchup_player_scores').select('*').eq('week_id', week.id).order('roster_slot'),
      supabase.from('manager_win_probabilities').select('*').eq('week_id', week.id),
      supabase.from('practice_simulation_events').select('*').eq('week_id', week.id).order('created_at'),
      supabase.from('weekly_recaps').select('*').eq('week_id', week.id).maybeSingle(),
    ])
    const firstError = [playerResult, pickResult, captainResult, scoreResult, resultResult, matchupResult, probabilityResult, eventResult, recapResult].find((item) => item.error)?.error
    if (firstError) { setError(firstError.message); return }
    setState({ week, profiles: profiles ?? [], players: playerResult.data ?? [], picks: pickResult.data ?? [], captains: captainResult.data ?? [], scores: scoreResult.data ?? [], results: resultResult.data ?? [], matchup: matchupResult.data ?? [], probabilities: probabilityResult.data ?? [], events: eventResult.data ?? [], recap: recapResult.data ?? null })
  }, [])
  ...
  const completedStages = new Set(state.events.map((event) => event.stage))
  const nextStage = !completedStages.has('thursday') ? 'thursday' : !completedStages.has('sunday') ? 'sunday' : !completedStages.has('monday') ? 'monday' : null
  ...
  const alternating = state.picks.every((pick, index) => pick.manager_id === (index % 2 === 0 ? state.week?.first_manager_id : state.profiles.find((item) => item.id !== state.week?.first_manager_id)?.id))
  const readiness = [
    ['Draft integrity', state.picks.length === 14 && alternating, `${state.picks.length}/14 alternating picks`],
    ['Roster shape', state.profiles.length === 2 && state.profiles.every((profile) => state.picks.filter((pick) => pick.manager_id === profile.id).length === 7), 'Seven legal slots per manager'],
    ['Captain locks', state.captains.length === 2, `${state.captains.length}/2 final choices`],
    ['Game windows', state.events.length === 3, `${state.events.length}/3 simulated`],
    ['Official result', state.week?.status === 'final' && state.results.length === 2, state.week?.status === 'final' ? 'Both results recorded' : 'Pending final scores'],
    ['Weekly recap', Boolean(state.recap), state.recap ? 'Scroober report generated' : 'Pending final whistle'],
  ]
  ...
  return <div className="test-lab"><header className="draft-nav"><button onClick={onBack} className="back-button">←</button><div><p className="company">Commissioner tools</p><h1>Practice Lab</h1></div><span className="mock-badge">Isolated test data</span></header>
    {!state.week ? <section className="test-intro"><p className="eyebrow">Safe sandbox</p><h2>Run a full week by yourself.</h2><p>This creates a separate practice matchup. Nothing here changes the real season or Luke’s permissions.</p><div><button disabled={busy} onClick={() => run('start_practice_week')}>Start manual test</button><button disabled={busy} onClick={() => run('run_practice_readiness_test')}>{busy ? 'Running full test…' : 'Run full readiness test'}</button></div></section> : <>
      ...
      <aside className="test-teams">{state.profiles.map((profile) => { const roster = state.picks.filter((pick) => pick.manager_id === profile.id); const score = state.scores.find((item) => item.manager_id === profile.id); const result = state.results.find((item) => item.manager_id === profile.id); const captainLocked = state.captains.some((item) => item.manager_id === profile.id); return <section className="test-panel" key={profile.id}><p className="eyebrow">{profile.display_name}</p><h3>{Number(score?.fantasy_points ?? 0).toFixed(2)} pts {result && `· ${result.result}`}</h3>{roster.map((pick) => { const info = player(pick.player_id); const captain = state.captains.some((item) => item.manager_id === profile.id && item.player_id === pick.player_id); return <div className="test-roster-player" key={pick.player_id}><span>{pick.roster_slot}</span><strong>{info?.full_name ?? pick.player_id}{pick.is_auto_pick ? ' · AUTO' : ''}</strong><button className={captain ? 'active' : ''} disabled={busy || state.picks.length < 14 || captainLocked} title={captainLocked ? 'Captain locked' : 'Lock this captain'} onClick={() => run('practice_select_captain', { p_manager_id: profile.id, p_player_id: pick.player_id })}>★</button></div>})}</section> })}</aside></div>
      {state.recap && <section className="simulation-recap"><p className="eyebrow">The Scroober Scrimage Report</p><h2>{state.recap.headline}</h2><p>{state.recap.summary}</p><div><span><small>Matchup MVP</small><strong>{state.recap.mvp_name}</strong><b>{state.recap.mvp_points} pts</b></span><span><small>Best value</small><strong>{state.recap.best_value_name}</strong><b>{state.recap.best_value_points} pts</b></span><span><small>Captain boost</small><strong>{state.recap.captain_name || 'No captain'}</strong><b>+{state.recap.captain_bonus} pts</b></span></div></section>}
    </>}
  </div>
}
