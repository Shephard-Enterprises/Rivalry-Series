...
export default function TestLab({ onBack }) {
  ...
  const load = useCallback(async () => {
    ...
    const [playerResult, pickResult, captainResult, scoreResult, resultResult] = await Promise.all([
      supabase.from('week_players').select('player_id, ranking, nfl_players!inner(full_name, position, nfl_team, headshot_url)').eq('week_id', week.id).eq('available', true).order('ranking', { nullsFirst: false }).limit(80),
  ...
  return <div className="test-lab"><header className="draft-nav"><button onClick={onBack} className="back-button">←</button><div><p className="company">Commissioner tools</p><h1>Practice Lab</h1></div><span className="mock-badge">Isolated test data</span></header>
    {!state.week ? <section className="test-intro"><p className="eyebrow">Safe sandbox</p><h2>Run a full week by yourself.</h2><p>This creates a separate practice matchup. Nothing here changes the real season or Luke’s permissions.</p><button disabled={busy} onClick={() => run('start_practice_week')}>Start practice week</button></section> : <>
      ...
      <div className="test-grid"><section className="test-panel"><p className="eyebrow">Manual alternating draft</p><h3>{state.picks.length < 14 ? `${nextManager?.display_name ?? 'Manager'} picks next` : 'Draft complete'}</h3><div className="test-player-list">{state.players.filter((item) => !drafted.has(item.player_id)).slice(0, 30).map((item) => <button disabled={busy || state.picks.length === 14} onClick={() => run('practice_make_pick', { p_player_id: item.player_id })} key={item.player_id}><span>{item.nfl_players.position}</span><strong>{item.nfl_players.full_name}</strong><small>{item.nfl_players.nfl_team}</small></button>)}</div></section>
