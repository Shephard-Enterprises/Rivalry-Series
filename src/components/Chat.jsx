...
export default function Chat({ social, onBack }) {
  ...
  return <div className="chat-page"><header className="draft-nav"><button onClick={onBack} className="back-button">←</button><div><p className="company">Rivalry chat</p><h1>Week {social.week?.nfl_week ?? 1} Thread</h1></div><span className="mock-badge">Private · Justin & Luke</span></header>
    <section className="chat-shell"><div className="chat-heading"><div><span className="live-pill"><i /> Realtime</span><h2>Talk your talk.</h2><p>Messages stay with this week’s matchup.</p></div></div>
      ...
      <form className="message-form" onSubmit={submit}>{(replyTo || editing) && <div className="composer-context"><span>{editing ? 'Editing your message' : `Replying to ${replyTo.sender_id === social.profile?.id ? 'yourself' : replyTo.profiles.display_name}`}</span><button type="button" onClick={cancelComposerMode}>×</button></div>}<label htmlFor="rivalry-message">Message Luke</label><div><button className="gif-button" type="button" onClick={() => setGifOpen(true)}>GIF</button><textarea id="rivalry-message" value={message} maxLength={1000} rows="2" placeholder="Say something memorable…" onChange={(event) => setMessage(event.target.value)} /><button disabled={sending || !message.trim()}>{sending ? 'Sending…' : editing ? 'Save' : 'Send'}</button></div><span>{message.length}/1000</span></form>
